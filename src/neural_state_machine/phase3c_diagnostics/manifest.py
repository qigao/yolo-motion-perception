...
from .contracts import registered_config, validate_registered_config
...
def build_input_manifest(
    ...
    *,
    ...
    formal_payload = json.loads(safe_path(root, "docs/experiments/phase-3c-formal-contract.json").read_text(encoding="utf-8"))
    if not isinstance(formal_payload, dict) or formal_payload.get("commit") != FORMAL_COMMIT:
        raise RuntimeError("formal contract binding differs from the registered commit")
    evidence_payload = json.loads(safe_path(root, FROZEN_PHASE3C_EVIDENCE).read_text(encoding="utf-8"))
    if not isinstance(evidence_payload, dict) or evidence_payload.get("config") != registered_config():
