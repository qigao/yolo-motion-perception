...
from neural_state_machine.r1_e1_evidence import (
    EvidenceInvalid,
    prepare_prospective,
    verify_evidence,
    write_measurement,
)
from neural_state_machine.r1_e1_protocol import protocol_smoke, run_registered_measurement
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="R1 E1 reservoir protocol and evidence tooling")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "prepare":
        prepared = prepare_prospective(args.output, scientific_head=args.scientific_head)
        _print(prepared)
        return 0

    if args.command == "verify":
        try:
            verified = verify_evidence(args.root, no_result_ok=args.no_result_ok)
