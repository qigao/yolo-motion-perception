...
def _cli():
    from scripts import benchmark_r1_e3_real_track as cli
...
def _forbidden_measurement(*args, **kwargs):
    pytest.fail("registered measurement was invoked before preflight completed")
...
def test_measure_rejects_existing_result_before_measurement(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    cli = _cli()
...
def test_measure_calls_registered_runner_only_after_all_preflight(
    ...
    monkeypatch.setattr(cli, "run_registered_measurement", run_measurement)
...
def test_permanent_ci_runs_focused_e3_tests_but_never_extracts_or_measures() -> None:
    workflow = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")

    assert "R1 E3 focused tests" in workflow
    assert "tests/test_r1_e3_" in workflow
